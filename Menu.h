#include "SeqList.c"

void Menu() //用户界面
{
	system("cls");
	printf("			*********************Sequence List System*********************\n");
	printf("			------------------------------   ------------------------------\n");
	printf("			**************************************************************\n");

	printf("			$                                                            $\n");
	printf("			$      1. Create List           2. Show List                 $\n");

	printf("			$                                                            $\n");
	printf("			$      3. Clear List            4. Sort List                 $\n");
	
	printf("			$                                                            $\n");
	printf("			$      5. Search by Location    6. Search by Value           $\n");

	printf("			$                                                            $\n");
	printf("			$      7. Insert by Location    8. Delete by Location        $\n");

	printf("			$                                                            $\n");
	printf("			$      9. Delete by Value       10. Delete Duplicate         $\n");

	printf("			$                                                            $\n");
	printf("			$      11. Reverse List         12. Partition List           $\n");

	printf("			$                                                            $\n");
	printf("			$      13. Import from File     14. Export to File           $\n");

	printf("			$                                                            $\n");
	printf("			$      15. Max Platform         16. Exit(0 for Chinese Menu) $\n");

	printf("			$                                                            $\n");
	printf("			------------------------------   ------------------------------\n");
	printf("			***************版权所有 洪智豪 杭州电子科技大学***************\n");
	
}